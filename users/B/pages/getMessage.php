<?php
require '../../../action.php';
// $qrry = 'SELECT * FROM messages WHERE userFk = 2';
// $total_messagess = mysqli_query($conn,$qrry);
// $total_messages = mysqli_num_rows($total_messagess);
// $_SESSION['total_messages'] = $total_messages;

$selectmsgs = "SELECT * FROM messages WHERE userFk = 2 order by id DESC";
...
if ($query->num_rows > 0) {
    while($row = mysqli_fetch_array($query)){
        ...
        echo "<tr style='background-color:$color'>
            <td class='mailbox-name'><a href='./readMailInbox.php?id=$X'>Manager</a></td>
