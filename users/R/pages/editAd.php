<?php 
  ...
  $tangazoId = $_SESSION['tangazoId'];
  $getCompany = "SELECT * FROM matangazo";
  $results = mysqli_query($conn,$getCompany);
  $getTangazoData = "SELECT * FROM matangazo WHERE id = '$tangazoId'";
  $resultsTangazo = mysqli_query($conn,$getTangazoData);
?>
<!DOCTYPE html>
<html>
<head>
...
$url1 = '../../../action.php';
$forLink = $tangazoId;
$url = add_or_update_params($url1,'tId',$forLink);
$link = $url;

echo '<form action="'.$link.'" method="post">';
                if ($resultsTangazo->num_rows > 0) { 
                    while($row = $resultsTangazo->fetch_assoc()) {
                        $adName = $row["tangazoName"];
                        $cost = $row["cost"];
                        $from = $row["playFrom"];
                        $to = $row["playTo"];
                ...
                echo '<div class="form-group">
                  <label>Ad Name</label>
                  <input type="text" name="tname" value="'.$adName.'" class="form-control" placeholder="Enter ...">
                </div>
                <div class="form-group">
                  <label>Ad Cost</label>
                  <input type="number" name="tcost" value="'.$cost.'" class="form-control" placeholder="Enter ...">
                </div>
                <!-- Date range -->
                <div class="form-group">
                  <label>Ad Play Date range: FROM - TO</label>
                  ...
                  <div class="input-group">
                    ...
                    <input type="text" value="'.$from.' - '.$to.'" name="fromto" class="form-control float-right" id="reservation">
                  </div>';
                    }
                }
?>
