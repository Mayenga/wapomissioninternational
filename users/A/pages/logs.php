<?php 
  ...
  //Retrieve all logs
  $id = $_SESSION['userId'];
  $sql ="SELECT * FROM logs WHERE userFK = '$id'";
  $query_runOne = mysqli_query($conn,$sql);
                  ...
                  <?php 
                      ...
                      if($upOne > 0){
                        while($rows = mysqli_fetch_array($query_runOne)){
                          echo "<tr><td>" . $NO. "</td><td>". $rows["datee"]."</td><td><a>".$rows["action"]."</a></td>";
                          $NO++;
                        }
                      }
                    $conn->close();
