...
<div class="wrapper">
  ...
  <li class="nav-item dropdown">
    ...
    <li class="nav-item dropdown">
      ...
      <li class="nav-item">
        <h4 style="color:green">Director</h4>
      </li>
    </ul>
  </nav> 
  <!-- /.navbar -->
              ...
              <div class="table-responsive mailbox-messages">
                <table class="table table-hover table-striped">
                  ...
                  <?php 
                    ...
                    $query = "SELECT * FROM messages WHERE userFk = '$user' ORDER BY id DESC";
                    $query_runOne = mysqli_query($conn,$query);
                    $upOne = mysqli_num_rows($query_runOne); 
                    $more = "B Shop";
                    if($upOne > 0){
                      while($rows = mysqli_fetch_array($query_runOne)){
                        $X = $rows['id'];
                        $Weddingdate = new DateTime($rows["time"]);
                        $mstime = $Weddingdate->format('d-m-Y H:i');
                        echo "<tr><td class='mailbox-name'><a href='./readMailSent.php?id=$X'>$more</a></td>";
                        echo '<td class="mailbox-subject"><b>'.$rows['subject'].'</b></td>';
                        echo '<td class="mailbox-date">'.$mstime.'</td></tr>';
                      }
                  }else{
                    echo '<tr><td>No Sent Messages</td></tr>';
                  }
                  ?>
                  </tbody>
                </table>
